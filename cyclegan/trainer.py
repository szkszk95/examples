...
datasetA = DATASET('/home/szk/PycharmProjects/pytorch-CycleGAN-and-pix2pix/datasets/horse2zebra/trainA', 143, 128, 1)
datasetB = DATASET('/home/szk/PycharmProjects/pytorch-CycleGAN-and-pix2pix/datasets/horse2zebra/trainB', 143, 128, 1)
...
for niter in range(opt.niter):
    ...
    # GAN LOSS
    err_ABA = BCELoss(out_fake_ABA, label_f)
    err_BAB = BCELoss(out_fake_BAB, label_f)
    err_GAN = err_ABA + err_BAB

    # MSE LOSS
    dis_A = L1Loss(fake_ABA, imageA)
    dis_B = L1Loss(fake_BAB, imageB)
    err_MSE = (dis_A + dis_B) * 10.0
    ...
    # tensorboardX
    writer.add_scalar('netD_real', err_real.data[0], niter)

    print('[{}/{}] netD_real {:.4f} netD_fake {:.4f} netG_GAN {:.4f} netG_MSE {:.4f}'.format(
        niter,
        opt.niter,
        err_fake.data[0], err_real.data[0], err_GAN.data[0], err_MSE.data[0]))

    if niter % 10 == 0:
        vutils.save_image(imageA, '{}/epoch_{}_A_real.png'.format(opt.outf, 500), normalize=True)
        vutils.save_image(imageB, '{}/epoch_{}_B_real.png'.format(opt.outf, 500), normalize=True)
        vutils.save_image(fake_AB, '{}/epoch_{}_A2B_fake.png'.format(opt.outf, 500), normalize=True)
        vutils.save_image(fake_BA, '{}/epoch_{}_B2A_fake.png'.format(opt.outf, 500), normalize=True)
        vutils.save_image(fake_ABA, '{}/epoch_{}_A2B2A_fake.png'.format(opt.outf, 500), normalize=True)
        vutils.save_image(fake_BAB, '{}/epoch_{}_B2A2B_fake.png'.format(opt.outf, 500), normalize=True)
